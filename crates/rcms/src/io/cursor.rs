...
mod tests {
    ...
    fn read_u16_array_matches_oracle() {
        ...
        for i in 0..n {
            expect.push(rcms_oracle::read_u16(&data[i * 2..]).unwrap());
        }
        assert_eq!(got, expect);
        // Also compare against the array oracle primitive directly.
        assert_eq!(got, rcms_oracle::read_u16_array(&data, n).unwrap());
        assert_eq!(r.tell(), 8);
    }
    ...
    fn read_u32_array_matches_oracle() {
        ...
        let mut expect = Vec::new();
        for i in 0..n {
            expect.push(rcms_oracle::read_u32(&data[i * 4..]).unwrap());
        }
        assert_eq!(got, expect);
        assert_eq!(r.tell(), 12);
    }
    ...
    fn read_alignment_matches_oracle() {
        ...
        for offset in 0u32..8 {
            let (oracle_ok, oracle_tell) = rcms_oracle::read_alignment(&data, offset);
            let mut r = MemReader::new(&data);
    ...
    fn read_alignment_truncated_matches_oracle() {
        ...
        let data = [0u8; 6];
        let offset = 5u32;
        let (oracle_ok, _) = rcms_oracle::read_alignment(&data, offset);
    ...
    fn read_type_base_matches_oracle() {
        ...
        assert_eq!(sig.to_raw(), rcms_oracle::read_type_base(&data));
