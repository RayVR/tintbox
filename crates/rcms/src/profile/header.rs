...
mod tests {
    ...
    fn header_matches_oracle_over_testbed() {
        ...
        for path in &entries {
            ...
            let oracle = rcms_oracle::read_header(&bytes);
            ...
            match (oracle, rust) {
                ...
                (Some(_), Err(e)) => {
                    panic!("lcms2 accepted but rcms rejected {}: {e}", path.display())
                }
                (None, Ok(_)) => panic!("rcms accepted but lcms2 rejected {}", path.display()),
            }
            compared += 1;
        }
