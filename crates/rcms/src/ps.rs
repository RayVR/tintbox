...
//! inherently non-deterministic, so rcms emits the header only behind the same gate
...
/// `NODEFAULTRESOURCEDEF`). rcms emits a placeholder timestamp; callers needing
