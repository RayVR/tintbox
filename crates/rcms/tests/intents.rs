...
//! (`rcms::link::default_icc_intents`) — the FIRST END-TO-END TRANSFORM.
//!
//! For every compatible ordered pair of testbed profiles, under
//! **RelativeColorimetric with BPC off**, build the rcms device-link pipeline via
...
//! must rcms, and the pixels must match to the bit.
...
fn default_icc_intents_relative_matches_oracle_over_testbed_pairs() {
    ...
    // Load every profile lcms2 accepts AND rcms parses.
    ...
    for a in &loaded {
        for b in &loaded {
            ...
            let oracle_out = match oracle_out {
                Some(o) => o,
                None => continue, // lcms2 rejected this chain; rcms isn't required to link it.
            };

            // lcms2 linked it: rcms MUST link it too.
            let lut = default_icc_intents(&[&pa, &pb], &[intent, intent], &bpc, &adaptation, 0)
                .unwrap_or_else(|e| {
                    panic!("lcms2 linked {} -> {} but rcms failed: {e}", a.name, b.name)
                });
