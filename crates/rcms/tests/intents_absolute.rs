...
//! perceptual / saturation intent routing in `rcms::link::default_icc_intents`
//! (slice-5 T3).
//!
//! For every compatible ordered pair of testbed profiles, build the rcms
...
//! T5 (deferred). So this test applies the SAME mutation rcms's `Transform` will
...
use rcms::link::{default_icc_intents, link_bpc_mutation};
...
/// Sweep every compatible ordered testbed pair under `intent`, comparing rcms's
...
fn sweep_intent(intent: RenderingIntent, requested_bpc: bool) -> (usize, usize, usize) {
    ...
    for a in &loaded {
        for b in &loaded {
            ...
            let oracle_out = rcms_oracle::transform_eval_float(
                &[&a.bytes, &b.bytes],
                &intents_raw,
                &bpc,
                &adaptation,
            ...
            let lut = default_icc_intents(&profiles, &[intent, intent], &bpc, &adaptation, 0)
                .unwrap_or_else(|e| {
                    panic!(
                        "lcms2 linked {} -> {} ({intent:?}) but rcms failed: {e}",
                        a.name, b.name
                    )
                });
