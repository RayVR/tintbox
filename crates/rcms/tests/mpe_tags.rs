...
//!      the tag via rcms `read_tag` -> `Tag::Lut(Pipeline)`, build the SAME
...
fn diff_mpet(bytes: &[u8], raw: u32, name: &str) -> usize {
    ...
    let sig = rcms::sig::Signature::from_raw(raw);
    ...
    let (c_in, c_out) = rcms_oracle::lut_channels(bytes, raw).expect("oracle lut channels");
    assert_eq!(
    ...
    // ---- 16-bit domain ----
    let oracle16 =
        rcms_oracle::lut_eval16(bytes, raw, &flat_u16, n_samples, n_out).expect("oracle eval16");
    ...
    let oraclef =
        rcms_oracle::lut_eval_float(bytes, raw, &flat_f32, n_samples, n_out).expect("oracle evalf");
...
fn mpet_tag_pipelines_match_oracle_over_testbed() {
    ...
    for path in &files {
        ...
        for sig in p.tags().collect::<Vec<_>>() {
            ...
            // In that case rcms must ALSO reject the tag (with a non-Unsupported
            ...
            match rcms_oracle::lut_channels(&bytes, raw) {
                Some(_) => {
                    assert!(
                        matches!(rust, Ok(Tag::Lut(_))),
                        "{name}:{raw:08x} lcms2 read the mpet pipeline but rcms did not: {rust:?}"
                    );
                ...
                None => {
                    assert!(
                        rust.is_err(),
                        "{name}:{raw:08x} lcms2 rejected the mpet tag but rcms accepted it: {rust:?}"
                    );
                    assert!(
                        !matches!(rust, Err(rcms::error::Error::Unsupported(_))),
...
fn synthetic_mpet_all_elements_match_oracle() {
    ...
    assert_eq!(
        rcms_oracle::tag_true_type(&profile, TAG_D2B0),
        Some(TY_MPE),
...
fn synthetic_mpet_nonsquare_matrix_matches_oracle() {
    ...
    assert_eq!(
        rcms_oracle::lut_channels(&profile, TAG_D2B0),
        Some((2, 4)),
...
fn mpet_type_no_longer_unsupported() {
    ...
    for path in testbed_icc() {
        ...
        for sig in p.tags().collect::<Vec<_>>() {
            let raw = sig.to_raw();
            if rcms_oracle::tag_true_type(&bytes, raw) != Some(TY_MPE) {
                continue;
            }
