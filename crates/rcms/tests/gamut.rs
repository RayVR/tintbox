//! Differential tests for `rcms::gamut` (lcms2 `cmssm.c` + `cmsgmt.c`):
...
use rcms::format::decode::{TYPE_CMYK_16, TYPE_RGB_16};
use rcms::gamut::{detect_tac, GamutBoundaryDescriptor};
use rcms::prelude::CIELab;
...
fn detect_tac_bit_exact_vs_lcms2() {
    ...
    for name in ["test1.icc", "test2.icc", "test3.icc", "crayons.icc"] {
        ...
        let theirs = rcms_oracle::detect_tac(&bytes);
        assert_eq!(
            mine.to_bits(),
            theirs.to_bits(),
    ...
    for &n_intent in &ALL_INTENTS {
        for &proof_intent in &ALL_INTENTS {
            for &gamutcheck in &[false, true] {
                ...
                let ok = rcms_oracle::proofing_transform_packed(
                    &in_bytes,
                    &out_bytes,
                    &proof_bytes,
                    n_intent.to_raw(),
...
fn proofing_transform_custom_alarm_codes_byte_identical() {
    ...
    let ok = rcms_oracle::proofing_transform_packed_alarm(
        &in_bytes,
        &out_bytes,
        &proof_bytes,
        intent.to_raw(),
...
fn gbd_check_point_matches_lcms2() {
    ...
    let theirs = rcms_oracle::gbd_check(&add, &check);
