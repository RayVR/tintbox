...
//! + the oracle extractors) and rcms `Profile::read_tag` decode it identically.
//!
...
fn named_color2_matches_oracle() {
    ...
    let oracle = rcms_oracle::read_tag_named_color2(&bytes, 0x6E63_6C32).expect("oracle ncl2");
    ...
    // vendorFlag is discarded by lcms2; verify rcms kept the raw on-disk value.
...
fn profile_sequence_desc_matches_oracle() {
    ...
    let oracle = rcms_oracle::read_tag_seq_desc(&bytes, 0x7073_6571).expect("oracle pseq");
    let p = Profile::open(&bytes).expect("rcms open");
    let items = match p
...
fn profile_sequence_id_matches_oracle() {
    ...
    let oracle = rcms_oracle::read_tag_seq_id(&bytes, 0x7073_6964).expect("oracle psid");
    let p = Profile::open(&bytes).expect("rcms open");
    let items = match p
...
fn dictionary_reclen32_matches_oracle() {
    ...
    // lcms2 (and rcms) carry the dict TYPE under the `meta` TAG; there is no
    ...
    let mut oracle = rcms_oracle::read_tag_dict(&bytes, meta_tag).expect("oracle dict");
