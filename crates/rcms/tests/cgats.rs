...
//!  1. Parse with rcms `Profile::load_from_mem` and lcms2 `cmsIT8LoadFromMem`,
...
fn assert_parity(label: &str, src: &str) {
    ...
    let mut rcms = Profile::load_from_mem(bytes)
        .unwrap_or_else(|e| panic!("[{label}] rcms failed to parse: {e:?}"));
    ...
    for t in 0..lcms.table_count() {
        rcms.set_table(t).expect("rcms set_table");
        ...
        let rcms_props: Vec<String> = rcms
            .enum_properties()
            .iter()
            .map(|s| s.to_string())
            .collect();
        ...
        for key in &rcms_props {
            ...
            assert_eq!(
                rcms.get_property(key).map(|s| s.to_string()),
                lcms.get_property(key),
                "[{label}] table {t} property '{key}' value"
            );
            assert_eq!(
                rcms.get_property_dbl(key).to_bits(),
        ...
        // DATA_FORMAT.
        let n_samples = rcms.num_samples();
        assert_eq!(
            n_samples,
            lcms.num_samples(),
        ...
        for c in 0..n_samples {
            assert_eq!(
                rcms.data_format(c).map(|s| s.to_string()),
                lcms.data_format(c),
        ...
        let n_patches = rcms.get_property_dbl("NUMBER_OF_SETS") as i32;
        for r in 0..n_patches {
            for c in 0..n_samples {
                assert_eq!(
                    rcms.get_data_rowcol(r, c).map(|s| s.to_string()),
                    lcms.get_data_rowcol(r, c),
                    "[{label}] table {t} data[{r}][{c}] string"
                );
                assert_eq!(
                    rcms.get_data_rowcol_dbl(r, c).to_bits(),
            ...
            // Patch name by index.
            assert_eq!(
                rcms.patch_name(r).map(|s| s.to_string()),
                lcms.patch_name(r),
        ...
        for r in 0..n_patches {
            if let Some(patch) = rcms.patch_name(r).map(|s| s.to_string()) {
                for c in 0..n_samples {
                    if let Some(sample) = rcms.data_format(c).map(|s| s.to_string()) {
                        assert_eq!(
                            rcms.get_data_by_name(&patch, &sample)
                                .map(|s| s.to_string()),
                            lcms.get_data(&patch, &sample),
                            "[{label}] table {t} data['{patch}']['{sample}'] string"
                        );
                        assert_eq!(
                            rcms.get_data_dbl(&patch, &sample).to_bits(),
    ...
    // Byte-exact save.
    let rcms_bytes = rcms.save_to_mem();
...
fn format_g_matches_c() {
    ...
    for &(x, want) in cases {
        assert_eq!(rcms::cgats::format_g(x, 10), want, "format_g({x})");
    }
}
