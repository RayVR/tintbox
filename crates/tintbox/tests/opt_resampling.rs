//! Differential tests for tintbox's `Lcms2Compat` resampling/devicelink optimizers
//! (`tintbox::opt::resampling` + `tintbox::opt::joincurves`, lcms2
...
use tintbox::format::decode::{self, PixelFormat};
...
fn lcms2compat_resampling_bit_identical_to_lcms2_default_over_testbed() {
    ...
    // Cells where tintbox legitimately declines to build (the deferred black-point
        ...
        for b in &loaded {
            ...
            for &intent in &intents {
                ...
                for &(ik, ok) in &[
                    ...
                    let built = tintbox_oracle::do_transform_packed_default(
                        &[&a.bytes, &b.bytes],
                        &intents_raw,
                        &bpc,
                        &adapt,
                    ...
                    let xform = match Transform::new_simple_with_formats_strategy(
                        ...
                        Err(tintbox::Error::Unsupported(_)) => {
                            deferred += 1;
                            continue;
                        }
                        Err(e) => panic!(
