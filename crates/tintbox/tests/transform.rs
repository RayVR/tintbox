//! Differential sweep for `tintbox::transform::Transform` + `do_transform_float`/
...
//! ICC intents with **BPC off**, build the tintbox `Transform` (which applies the
...
use tintbox::format::PixelFormat;
    ...
    for a in &loaded {
        for b in &loaded {
            ...
            for &intent in &intents {
                ...
                // whether tintbox returns Unsupported.
                ...
                let oracle_f = tintbox_oracle::transform_eval_float(
                    &[&a.bytes, &b.bytes],
                    &intents_raw,
                    &bpc,
                    &adaptation,
                ...
                let oracle_f = match oracle_f {
                    Some(o) => o,
                    None => continue, // lcms2 rejected; tintbox isn't required to link it.
                };

                // Build the tintbox transform. For forced-BPC cells that still need
                // detection-by-sampling (V4 matrix-shaper perc/sat, etc.), tintbox
                ...
                let xform = match Transform::new_simple(&pa, &pb, intent, false) {
                    ...
                    Err(e) => panic!(
                        "lcms2 linked {} -> {} ({intent:?}) but tintbox Transform failed: {e}",
                        a.name, b.name
                    ),
                };

                let oracle_16 = tintbox_oracle::transform_eval_16(
                    &[&a.bytes, &b.bytes],
                    &intents_raw,
                    &bpc,
                    &adaptation,
...
/// float by identity (or /100 for ink). To feed the *same* numbers to both tintbox
        ...
        for b in &loaded {
            ...
            for &intent in &intents {
                ...
                for &(ik, ok) in &kind_pairs {
                    ...
                    let ok_built = tintbox_oracle::do_transform_packed(
                        &[&a.bytes, &b.bytes],
                        &intents_raw,
                        &bpc,
                        &adaptation,
                    ...
                    if !ok_built {
                        continue; // lcms2 rejected this chain; tintbox not required.
                    }
...
/// ONE pre-multiplied matrix. tintbox previously left them as two separate `Matrix`
...
/// 115 → 8→16 value 29555, output channel 1: tintbox 27417 vs lcms2 27419). The
...
fn enc_random_alpha(dst: &mut [u8], rng: &mut tintbox_oracle::Rng, fmt: u32) {
    let f = PixelFormat(fmt);
    if f.is_float() {
...
/// loadable testbed RGB→RGB (and a CMYK case) pairs, build the tintbox transform
...
fn copy_alpha_extra_channel_matches_lcms2() {
    ...
    let mut rng = tintbox_oracle::Rng::new(0x00A1_FA00);
    let n = 64usize; // pixels per cell
    let mut cells = 0usize;
    let mut alpha_samples = 0usize;

    let flags = tintbox::transform::Flags::NOOPTIMIZE.union(tintbox::transform::Flags::COPY_ALPHA);
        ...
        for b in &rgb {
            ...
            for &intent in &intents {
                ...
                for &(in_fmt, out_fmt) in rgba_pairs {
                    ...
                    let ok = tintbox_oracle::do_transform_packed_copyalpha(
                        &[&a.bytes, &b.bytes],
                        &intents_raw,
                        &[false, false],
                        &[1.0, 1.0],
...
fn copy_alpha_noop_when_no_extra_channels() {
    ...
    let with_alpha = Transform::new_simple_with_formats_flags(
        ...
        in_fmt,
        out_fmt,
        tintbox::transform::Flags::NOOPTIMIZE.union(tintbox::transform::Flags::COPY_ALPHA),
    )
    .unwrap();
