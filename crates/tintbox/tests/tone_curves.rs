...
use tintbox::curve::{build_parametric, build_tabulated_16, build_tabulated_float};
    ...
    for &n in TABLE_SIZES {
        ...
        for _ in 0..8 {
            ...
            while v <= 0xffff {
                ...
                let want = tintbox_oracle::tabulated16_eval16(&table, v as u16);
            ...
            for &v in &[0u16, 1, 0x7fff, 0x8000, 0xfffe, 0xffff] {
                ...
                let want = tintbox_oracle::tabulated16_eval16(&table, v);
    ...
    for &n in TABLE_SIZES {
        for _ in 0..4 {
            ...
            for &x in &xs {
                ...
                let want = tintbox_oracle::tabulated16_eval_float(&table, x);
                assert_eq!(
                    got.to_bits(),
                    want.to_bits(),
...
fn eval_float_tabulated_float_matches_oracle() {
    ...
    for &n in TABLE_SIZES {
        for _ in 0..4 {
            ...
            for &x in &xs {
                let got = curve.eval_float(x);
                let want = tintbox_oracle::tabulated_float_eval_float(&table, x)
...
fn eval_float_parametric_matches_oracle() {
    ...
    for &ty in PARAM_TYPES {
        for _ in 0..16 {
            ...
            for &x in &xs {
                let got = curve.eval_float(x);
                let want = match tintbox_oracle::parametric_eval_float(ty, &params, x) {
                    Some(w) => w,
                    None => continue,
                };
                assert_eq!(
...
fn parametric_table16_materialization_matches_oracle() {
    ...
    for &ty in PARAM_TYPES {
        for _ in 0..16 {
            ...
            let want = match tintbox_oracle::parametric_table16(ty, &params) {
                Some(w) => w,
                None => continue,
            };
            assert_eq!(
...
fn tabulated_float_table16_materialization_matches_oracle() {
    ...
    for &n in TABLE_SIZES {
        for _ in 0..4 {
            let table: Vec<f32> = (0..n).map(|_| rng.next_f64_unit() as f32).collect();
            let curve = build_tabulated_float(&table).expect("non-empty table");
            let want = tintbox_oracle::tabulated_float_table16(&table).expect("accepted");
...
fn gamma_table_node_count() {
    ...
    let g1 = tintbox::curve::build_gamma(1.0);
    assert_eq!(g1.table16().len(), 2, "gamma=1.0 should use 2 grid points");
    let g22 = tintbox::curve::build_gamma(2.2);
    assert_eq!(
        g22.table16().len(),
        4096,
    ...
    let want = tintbox_oracle::parametric_table16(1, &[1.0]).expect("accepted");
...
fn is_linear_on_known_curves() {
    ...
    assert!(tintbox::curve::build_gamma(1.0).is_linear());
    assert!(!tintbox::curve::build_gamma(2.2).is_linear());
}
...
fn is_monotonic_on_known_curves() {
    ...
    assert!(tintbox::curve::build_gamma(2.2).is_monotonic());
}
