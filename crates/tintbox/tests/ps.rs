...
fn assert_bytes_eq(got: &[u8], want: &[u8], ctx: &str) {
    assert_eq!(
        got.len(),
        want.len(),
        "{ctx}: length mismatch tintbox={} lcms2={}",
        got.len(),
        want.len()
    );
    if got != want {
...
fn csa_byte_identical() {
    ...
    for &name in PROFILES {
        ...
        // Skip profiles tintbox can't even open (none expected, but be robust).
        ...
        for intent in 0u32..4 {
            let want = tintbox_oracle::get_postscript_csa(&bytes, intent, 0);
            ...
            match (got, want) {
                ...
                (Ok(_), None) => {
                    panic!("CSA {name} intent={intent}: tintbox emitted, lcms2 declined")
                }
                (Err(e), Some(_)) => {
                    panic!("CSA {name} intent={intent}: tintbox declined ({e:?}), lcms2 emitted")
                }
            }
        }
    }
    ...
    for &name in PROFILES {
        ...
        for intent in 0u32..4 {
            for &extra in &[0u32, 0x2000 /* BPC */] {
                ...
                let want = tintbox_oracle::get_postscript_crd(&bytes, intent, flags);
                ...
                match (got, want) {
                    ...
                    (Ok(_), None) => panic!(
                        "CRD {name} intent={intent} flags={flags:#x}: tintbox emitted, lcms2 declined"
                    ),
                    (Err(e), Some(_)) => panic!(
                        "CRD {name} intent={intent} flags={flags:#x}: tintbox declined ({e:?}), lcms2 emitted"
                    ),
                }
            }
        }
