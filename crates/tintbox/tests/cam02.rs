//! Differential test for the CIECAM02 appearance model (`tintbox::cam02`) against
...
//! - Forward: tintbox JCh == lcms2 JCh (each of J, C, h bit-exact),
//! - Reverse: tintbox XYZ == lcms2 XYZ over the JCh produced by the forward pass.
//!
//! lcms2 uses `f64` throughout and tintbox transcribes the same operation order, so
...
use tintbox::color::{JCh, CIEXYZ};
use tintbox_oracle::OracleCam02;
        ...
        for xyz in &samples {
            ...
            assert!(
                ...
                "Forward mismatch wp={wp:?} Yb={yb} La={la} surround={surround} D={d_value} \
                 XYZ={xyz:?}: tintbox JCh=({},{},{}) [{:#018x},{:#018x},{:#018x}] vs \
            ...
            assert!(
                ...
                "Reverse mismatch wp={wp:?} Yb={yb} La={la} surround={surround} D={d_value} \
                 JCh=({},{},{}): tintbox XYZ=({},{},{}) [{:#018x},{:#018x},{:#018x}] vs \
                 lcms2=({},{},{}) [{:#018x},{:#018x},{:#018x}]",
                r_jch.j,
                r_jch.c,
                r_jch.h,
