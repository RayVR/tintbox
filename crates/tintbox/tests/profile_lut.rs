...
//! (`tintbox::link::read_input_lut` / `read_output_lut` / `read_devicelink_lut`),
...
//! builds a LUT, build the tintbox pipeline too and evaluate BOTH at a grid of
...
fn classify_input(p: &Profile) -> &'static str {
    use tintbox::profile::ColorSpace;
    let h = p.header();
    let a2b = [b"A2B0", b"A2B1", b"A2B2"];
    let has_lut = a2b
        .iter()
        .any(|s| p.has_tag(tintbox::sig::Signature::from_bytes(**s)))
        || p.has_tag(tintbox::sig::Signature::from_bytes(*b"D2B0"));
    if has_lut {
        "lut"
...
fn sweep(
    which: ReadLut,
    rust_build: impl Fn(&Profile, u32) -> tintbox::Result<tintbox::pipeline::Pipeline>,
    ...
    for path in &files {
        ...
        for intent in 0u32..=3 {
            ...
            let (n_in, n_out) = match tintbox_oracle::read_lut_channels(&bytes, which, intent) {
                Some(c) => (c.0 as usize, c.1 as usize),
                None => {
                    // lcms2 returns NULL: tintbox must also fail to build (or build
            ...
            let oracle_out = tintbox_oracle::read_lut_eval_float(
                &bytes,
                which,
                intent,
                &flat_in,
...
/// Differential: `tintbox::curve::reverse_tone_curve` of a tabulated curve must
...
fn reverse_tone_curve_matches_oracle() {
    use tintbox::curve::{build_tabulated_16, reverse_tone_curve};
    use tintbox::profile::Tag;
    ...
    for path in testbed_icc() {
        ...
        for s in [b"rTRC", b"gTRC", b"bTRC", b"kTRC"] {
            let sig = tintbox::sig::Signature::from_bytes(*s);
            if !p.has_tag(sig) {
                continue;
            }
    ...
    for table in &tables {
        ...
        let oracle =
            tintbox_oracle::reverse_tabulated16_eval_float(table, &xs).expect("oracle rev");
...
fn input_sweep_covers_matrix_shaper_and_lut() {
    ...
    for path in testbed_icc() {
        ...
        if tintbox_oracle::read_lut_channels(&bytes, ReadLut::Input, 0).is_none() {
            continue;
        }
