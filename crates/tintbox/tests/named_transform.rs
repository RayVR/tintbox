...
//! `cmsCreateLab4Profile` bytes (via the oracle), which tintbox also opens.

use tintbox::named::NamedColorList;
use tintbox::profile::{Profile, RenderingIntent, Tag};
...
fn named_list_accessors_match_oracle() {
    ...
    let oracle = tintbox_oracle::read_tag_named_color2(&bytes, SIG_NCL2).expect("oracle ncl2");
...
fn named_device_transform_bit_exact() {
    ...
    // tintbox: single named-color profile, TYPE_NAMED_COLOR_INDEX -> 4ch device u16.
    let p = Profile::open(&bytes).expect("tintbox open");
    let xform = Transform::new_with_formats(
        ...
        &[1.0],
        Flags::NOOPTIMIZE,
        tintbox::format::TYPE_NAMED_COLOR_INDEX,
        u16_format(4),
    )
    .expect("tintbox named device xform");
    ...
    let oracle_out = tintbox_oracle::named_transform_16(
        &bytes,
        None,
...
fn named_pcs_transform_bit_exact() {
    ...
    let lab_bytes = tintbox_oracle::save_virtual_profile(RCMS_T4_LAB4).expect("lab4 profile bytes");

    let n_colors = spot_colors().len();
    let indices: Vec<u16> = (0..(n_colors as u16 + 2)).collect();

    let pn = Profile::open(&named_bytes).expect("tintbox open named");
    let pl = Profile::open(&lab_bytes).expect("tintbox open lab4");
    ...
    .expect("tintbox named pcs xform");
