...
//! - `tetrahedral_16` vs `tintbox_oracle::tetra16`, bit-exact u16, over random
...
//! - `tetrahedral_float` vs `tintbox_oracle::tetra_float`, bit-exact (f32::to_bits),
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in u16_inputs(&grid, &mut rng) {
                    ...
                    let oracle = tintbox_oracle::tetra16(&grid, n_out, &table, &input)
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in f32_inputs(&mut rng) {
                    ...
                    let oracle = tintbox_oracle::tetra_float(&grid, n_out, &table, &input)
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in u16_inputs_nd(2, &mut rng, 4000) {
                    ...
                    let oracle = tintbox_oracle::interp16(&grid, n_out, &table, 0, &input)
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in f32_inputs_nd(2, &mut rng, 1500) {
                    ...
                    let oracle = tintbox_oracle::interp_float(&grid, n_out, &table, 0, &input)
...
/// the grid (latent OOB; inputs are expected pre-clamped). tintbox clamps the index
/// to stay memory-safe, so at that exact band tintbox and the (OOB-reading) oracle
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in u16_inputs(&grid, &mut rng) {
                    ...
                    let oracle = tintbox_oracle::interp16(
                        &grid,
                        n_out,
                        &table,
        ...
        for &n_out in N_OUTS {
            ...
            for _ in 0..4 {
                ...
                for input in f32_inputs(&mut rng) {
                    ...
                    let oracle = tintbox_oracle::interp_float(
                        &grid,
                        n_out,
                        &table,
                        CMS_LERP_FLAGS_TRILINEAR,
