...
//!
//! For every `vendor/Little-CMS/testbed/*.icc` that both tintbox and lcms2 accept:
//!
//! 1. tintbox reads the profile, then [`build_writable`] reconstructs a
...
//!    fresh ICC bytes.
//! 2. tintbox reads those bytes BACK and asserts STRUCTURAL/semantic equality of every
...
//! 3. CROSS-CHECK: lcms2 reads the tintbox-written bytes and a few representative tag
//!    values (wtpt/desc/the LUT or TRC tags) match tintbox's parsed values — proving
//!    the tintbox-written profile is valid + correct for a third-party reader.
//!
//! Byte-vs-original is NOT the contract (lcms2 raw-copies unmodified disk tags;
//! tintbox re-serializes). The T0-T4 byte-identity tests cover byte exactness for the
...
use tintbox::error::Error;
use tintbox::profile::tag::Tag;
use tintbox::profile::{save_to_mem, Profile, SlotContent, WritableProfile};
...
///
/// tintbox reads these but slice-7 did not land writers for them (they are not
...
fn build_writable(p: &Profile, bytes: &[u8]) -> (WritableProfile, BTreeMap<u32, usize>) {
    ...
    for sig in p.tags().collect::<Vec<_>>() {
        ...
        if !writer_supports(*p.header(), sig, &value) {
            let ty = tintbox_oracle::tag_true_type(bytes, sig.to_raw()).unwrap_or(0);
...
fn writer_supports(header: tintbox::profile::Header, sig: Signature, value: &Tag) -> bool {
...
fn roundtrip_writer_reader_parity_over_testbed() {
    ...
    for path in &files {
        ...
        // 3. tintbox reads the written bytes back; assert it is a valid profile.
        let back = Profile::open(&written)
            .unwrap_or_else(|e| panic!("{name}: tintbox cannot re-open its own bytes: {e:?}"));
...
/// CROSS-CHECK: lcms2 reads the tintbox-WRITTEN bytes and the representative tag
/// values (wtpt / desc-family / the TRC or LUT tags) match tintbox's parsed values.
/// This proves the tintbox-written profile is valid and correct for a third-party
/// reader, not merely self-consistent under tintbox's own reader.
#[test]
fn lcms2_rereads_rcms_written_bytes_over_testbed() {
    ...
    for path in &files {
        ...
        // lcms2 must accept the tintbox-written bytes.
        assert!(
            tintbox_oracle::open_succeeds(&written),
            "{name}: lcms2 rejects tintbox-written bytes"
        );
        profiles_crosschecked += 1;

        // XYZ-valued tags: tintbox parse vs lcms2 reading the written bytes.
        for raw in [WTPT, BKPT, LUMI, RXYZ] {
            let sig = Signature::from_raw(raw);
            let Ok(Tag::Xyz(v)) = p.read_tag(sig) else {
                continue;
            };
            let Some(c) = tintbox_oracle::read_tag_xyz(&written, raw) else {
                continue;
            };
            tintbox_oracle::assert_f64_bits_eq(v.x, c[0], (name.as_str(), raw, "x"));
            tintbox_oracle::assert_f64_bits_eq(v.y, c[1], (name.as_str(), raw, "y"));
            tintbox_oracle::assert_f64_bits_eq(v.z, c[2], (name.as_str(), raw, "z"));
            xyz_checks += 1;
        }

        // TRC curves: lcms2 evaluates the tintbox-written curve; compare to tintbox's.
        for raw in [RTRC, KTRC] {
            let sig = Signature::from_raw(raw);
            let Ok(Tag::Curve(curve)) = p.read_tag(sig) else {
                continue;
            };
            let Some(cys) = tintbox_oracle::read_tag_curve(&written, raw, &xs) else {
                continue;
            };
            for (&x, &cy) in xs.iter().zip(cys.iter()) {
                ...
                assert_eq!(
                    ry.to_bits(),
                    cy.to_bits(),
                    "{name}:{raw:08x} curve x={x}: tintbox={ry} lcms2(tintbox-bytes)={cy}"
                );
            }
            curve_checks += 1;
        }

        // The A2B0 LUT: lcms2 evaluates the tintbox-written tag through its own LUT
        // reader; compare outputs to tintbox's pipeline over a small grid.
        if let Ok(Tag::Lut(lut)) = p.read_tag(Signature::from_raw(A2B0)) {
            if let Some((n_in, n_out)) = tintbox_oracle::lut_channels(&written, A2B0) {
                if n_in as usize == lut.input_channels && n_out as usize == lut.output_channels {
                    for input in lut_grid(lut.input_channels) {
                        ...
                        if let Some(c) = tintbox_oracle::lut_eval_float(
                            &written,
                            A2B0,
                            &input,
                            1,
                            ...
                            for (i, (ry, cy)) in rust.iter().zip(c.iter()).enumerate() {
                                assert_eq!(
                                    ry.to_bits(),
                                    cy.to_bits(),
                                    "{name}: A2B0 lcms2(tintbox-bytes) ch{i} at {input:?}"
                                );
                            }
                        }
                    }
    ...
    println!(
        "lcms2 re-read cross-check: {profiles_crosschecked} tintbox-written profiles accepted \
         by lcms2; {xyz_checks} XYZ, {curve_checks} curve, {lut_checks} LUT tag values matched"
    );
    assert!(
        profiles_crosschecked > 0,
        "expected lcms2 to accept some tintbox-written profiles"
    );
    assert!(
